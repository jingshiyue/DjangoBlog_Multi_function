...
class Article(BaseModel):
    ...
    def get_absolute_url(self):
        return reverse('blog:detailbyid', kwargs={ #反转找到path(r'article/<int:year>/<int:month>/<int:day>/<int:article_id>.html',，
            'article_id': self.id,               #path里的参数要与reverse里对应，多了少了都会报错
    ...
    def get_category_tree(self):
        print("blog.models.Article.get_category_tree.缓存方法")
    ...
    def next_article(self):
        print("blog.models.Article.next_article.缓存方法")
        # 下一篇
        return Article.objects.filter(id__gt=self.id, status='p').order_by('id').first()

    @cache_decorator(expiration=60 * 100)  #expiration 到期
    def prev_article(self):
        print("blog.models.Article.prev_article.缓存方法")
        # 前一篇
...
class Tag(BaseModel):
    ...
    slug = models.SlugField(default='no-slug', max_length=60, blank=True) #后面用exclude排出admin可以编辑区域
...
class BlogSettings(models.Model):
    ...
    def clean(self):   #当add实例时，先执行clean，再执行save
        # print("models.clean!!!!!!!!!+++++++++++++!")
