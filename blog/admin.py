...
class TagAdmin(admin.ModelAdmin):
    exclude = ('slug', 'last_mod_time', 'created_time', 'created_time111')
...
class SideBarAdmin(admin.ModelAdmin):
    list_display = ('name', 'content', 'is_enable', 'sequence',)  #显示处，而不是编辑model实例处
    exclude = ('last_mod_time', 'created_time')   #创建model实例时，这两个字段不能被编辑，使用默认
