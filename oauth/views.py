    ...
    if user:
        ...
        if user.email:
            with transaction.atomic(): #在执行上下文里面的内容时候时，遇到错误执行回滚操作，类似mysql回滚函数
                author = None
                try:
                    author = get_user_model().objects.get(id=user.author_id)  #通过认证模型的外键或者邮箱来查找用户模型，认证模型和用户模型邮箱名相同
                except ObjectDoesNotExist:
                    pass
                if not author:
                    result = get_user_model().objects.get_or_create(email=user.email)  #Returns a tuple of (object, created)
                    author = result[0]
                    if result[1]:
                ...
                oauth_user_login_signal.send(sender=authorize.__class__, id=user.id)  #这个信号用于登陆前，删除上一个用户的缓存信息
                login(request, author)  #用户模型登陆，先删除上一个用户的缓存，再通过外键或者邮箱得到用户模型，最终用用户模型登陆。
